...
library("dplyr")
library("SnowballC")
setwd('/Volumes/GoogleDrive/My Drive/ML_AI/Edureka/DS with R/Class 8')
getwd()
Review_doc <- readLines("RestReviews.tsv")
df <- data.frame( text = Review_doc, stringsAsFactors = FALSE)
#To break into words we would require Corpus
Rev_doc1 <- Corpus(VectorSource(Review_doc))
Rev_doc1<- tm_map(removePunctuation(Rev_doc1))
# Remove punctuations
Rev_doc1<- tm_map(removeNumbers(Rev_doc1))
%>% # Remove numbers
...
Review_matrix <- DocumentTermMatrix(Rev_doc1)
dtmdell
